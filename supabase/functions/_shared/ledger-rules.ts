...
export const PL_CREDIT_RULES: LedgerRule[] = [
  ...
  { pattern: /\bintt?\s+on\s+deposit/i, target: { statement: 'pl', key: 'other_income', noteHead: 'Interest income' } },
...
export const BS_RULES: LedgerRule[] = [
  { pattern: /\b(capital\s+account|owners?'?\s+capital|proprietor)\b/i, target: { statement: 'bs', side: 'equity_and_liabilities', key: 'owners_capital' } },
  { pattern: /\bunsecured\s+loan/i, target: { statement: 'bs', side: 'equity_and_liabilities', key: 'long_term_borrowings' } },
  { pattern: /\b(cc\s+limit|cash\s+credit|overdraft|secured\s+loan)\b/i, target: { statement: 'bs', side: 'equity_and_liabilities', key: 'short_term_borrowings' } },
  { pattern: /\b(sundry\s+creditor|trade\s+payable|goods\s+creditor)\b/i, target: { statement: 'bs', side: 'equity_and_liabilities', key: 'trade_payables' } },
  ...
  { pattern: /\b(fixed\s+asset|property.*equipment|schedule\s*[\"']?a)/i, target: { statement: 'bs', side: 'assets', key: 'property_plant_equipment' } },
