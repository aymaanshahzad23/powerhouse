...
export function LinkedInLink({ href, label, className }: LinkedInIconProps) {
  return (
    <a
      href={href}
      target="_blank"
      ...
      aria-label={label}
      className={cn(
        "group flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white/50 transition-all hover:border-[#0a66c2]/50 hover:bg-[#0a66c2]/15 hover:text-[#71b7ff]",
        className
      )}
    >
      <svg
